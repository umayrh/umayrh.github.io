...
![ISTA vs FISTA, 500 iterations](http://umayrh.files.wordpress.com/2014/07/ista500.png?w=300)
ISTA vs FISTA, 500 iterations

![ISTA vs FISTA, 5000 iterations](http://umayrh.files.wordpress.com/2014/07/ista5000.png?w=300)
...
domain $Q_p$. The representation is similar to the [convex conjugate](http://en.wikipedia.org/wiki/Convex_conjugate), $f\*(u)$, 
of this function. Indeed Nesterov considers using $phi{(u)} = f\*(u)$, but then argues that such $phi{(u)}$ might not 
be a simple enough function. This might be true in general, but as we'll see that in the special case of $L_1$ 
regularization, $phi{(u)} = f\*(u)$ will work.
...
[Theorem 1, [Nesterov05]], where $\|A\|$ is the operator norm of $A$, and (2) that $f \mu{(x)}$ is a uniform smooth approximation 
of $f(x)$ for $mu > 0$. So now we can use an optimal gradient descent for smooth optimization to minimize $f \mu{(x)}$.
...
Applying this algorithm to optimize $f \mu{(x)}$ Theorem 3, [Nesterov05] gives $O(1/k$ convergence for the optimal value of $\mu$ 
since: 
...
[Becker09] call their application of [Nesterov05] to $L_1$ regularization, NESTA. They show that:
  ...
  Choosing $prox \alpha{(u)}$ to be the squared Euclidean distance with $\alpha = 1$ (cf. Section 4.2, [Nesterov05]),
...
* https://github.com/gpeyre/numerical-tours/tree/master/matlab/solutions 
* https://www.ceremade.dauphine.fr/~peyre/numerical-tour/tours/optim_4_fb/#54 
* http://www.caam.rice.edu/~optimization/disparse/LASSO/FISTA/pFistaLasso.html 
* http://www.mathworks.com/matlabcentral/fileexchange/16204-toolbox-sparse-optmization/content/toolbox_optim/perform_fb.m 
* http://slipguru.disi.unige.it/Software/L1L2Py/algorithms.html 
* http://www.eecs.berkeley.edu/~yang/software/l1benchmark/ 
* http://www.ece.rice.edu/~tag7/Tom_Goldstein/CGIST.html 
* http://users.ece.gatech.edu/%7Esasif/index.html 
* https://www.ceremade.dauphine.fr/~peyre/numerical-tour/ 
* http://www.mit.edu/~dimitrib/PTseng/papers/apg_alg.m 
* http://www.ece.rice.edu/~tag7/Tom_Goldstein/CGIST.html 
